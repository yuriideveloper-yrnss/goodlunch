...
import { SmartCateringModal } from '@/components/smartcatering/SmartCateringModal'
import { CalorieAdvisor } from '@/components/ui/CalorieAdvisor'
import { trackEvent } from '@/lib/tracking'

export default function SmartCateringCalculator({ dict, lang }: { dict: any; lang?: string }) {
        ...
        <div className="max-w-3xl mx-auto bg-brand-bg rounded-3xl p-8 shadow-sm border border-gray-100">
          ...
          <div className="flex bg-white rounded-xl p-1 mb-8 shadow-sm relative">
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-lg text-sm font-bold transition-colors z-10 relative ${
                mealPackage === 'meals3' ? 'text-white' : 'text-gray-500 hover:text-gray-700'
              }`}
            >
              {mealPackage === 'meals3' && (
                <motion.div
                  layoutId="smartPackageTab"
            ...
            <button
              ...
              className={`flex-1 py-3 px-4 rounded-lg text-sm font-bold transition-colors z-10 relative ${
                mealPackage === 'meals4' ? 'text-white' : 'text-gray-500 hover:text-gray-700'
              }`}
            >
              {mealPackage === 'meals4' && (
                <motion.div
              ...
              {currentOptions.map((tier) => {
                ...
                return (
                  <button
                    ...
                    className={`flex-1 min-w-[80px] py-3 rounded-lg text-sm font-bold transition-colors relative z-10 ${
                      isActive ? 'text-white' : 'text-gray-600 hover:text-gray-800'
                    }`}
                  >
                    {isActive && (
                      <motion.div
                        layoutId="smartCalorieTab"
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-6 bg-white p-6 rounded-2xl border border-brand-orange/20">
            ...
            <motion.button
              ...
              className="w-full sm:w-auto bg-brand-orange text-white px-8 py-4 rounded-xl font-bold shadow-lg shadow-brand-orange/30 cursor-pointer"
            >
              {dict.calc.order_btn}
            </motion.button>
          </div>
